...
import { Loader2, MessageCircle, Star, ThumbsDown, ThumbsUp } from "lucide-react";
          ...
          <CardContent className="grid gap-4 p-6">
            ...
            {activeDeck && (
              <div className="space-y-6">
                <div className="rounded-xl border border-border/60 bg-surface-muted/40 p-6">
                  <div className="flex flex-wrap items-center justify-between gap-4">
                    ...
                    <div className="flex flex-col items-end gap-1 text-right">
                      ...
                      <span className="text-xl font-semibold text-accent">{activeDeck.deck.averageElixir.toFixed(1)}</span>
                    </div>
                  </div>

                  <div className="mt-6 grid gap-3 sm:grid-cols-2">
                    {Object.entries(activeDeck.breakdown).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between rounded-lg border border-border/60 bg-background/80 px-4 py-3">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    ...
                    <Button variant="outline" className="gap-2" onClick={startCheckout} disabled={checkoutLoading}>
                      {checkoutLoading ? <Loader2 className="size-4 animate-spin" /> : <Star className="size-4" />}
                      Unlock pro coaching
                    </Button>
                  </div>
                </div>
                ...
                {activeDeck.explainer && (
                  <div className="space-y-4 rounded-xl border border-border/60 bg-surface-muted/30 p-6">
          ...
          <CardContent className="space-y-6 p-6">
            ...
            <div className="rounded-lg border border-border/60 bg-surface-muted/40 p-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-text">Want a human coach?</p>
                  <p className="text-xs text-text-muted">Decksy Pro unlocks detailed matchup reviews and history tracking.</p>
                </div>
              </div>
              <Button className="mt-4 w-full" onClick={startCheckout} disabled={checkoutLoading}>
                {checkoutLoading ? <Loader2 className="size-4 animate-spin" /> : <Star className="size-4" />}
                Upgrade to Pro
              </Button>
            </div>
